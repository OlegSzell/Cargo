Public Class ЖурналПеревозчикСобытияТаблица
    ...
    Private Sub ЖурналПеревозчикСобытияТаблица_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim f = (From x In AllClass.ЖурналПеревозчик
                 ...
                 Where x.Код = ID
                 Select New Grid1Class With {.Дата = y.Дата, .Сообщение = y.Событие}).ToList()
        ...
        Using db As New dbAllDataContext()
            ...
            If f IsNot Nothing Then
                ...
                If lst IsNot Nothing Then
                    ...
                    For Each b In lst
                        Dim m As New ЖурналПеревозчикСобытия With {.IDЖурналПеревозчик = ID, .Дата = b.Дата, .Событие = b.Сообщение}
                        mnew.Add(m)
                    Next
